    ...
    return RouteWrapper({
        ...
        work: async () => {
            ...
            if (user) {
                ...
                // ProfileSkill
                const skill = {
                    gskill: Number(body.gskill * 100 || '0'),
                    dskill: Number(body.dskill * 100 || '0'),
                    gall: Number(body.gskillall * 100 || '0'),
                    dall: Number(body.dskillall * 100 || '0'),
                    gclearlv: Number(body.gclearlv * 100 || '0'),
                    dclearlv: Number(body.dclearlv * 100 || '0'),
                    gclearnum: Number(body.gclearnum || '0'),
                    dclearnum: Number(body.dclearnum || '0'),
                    gfclv: Number(body.gfclv * 100 || '0'),
                    dfclv: Number(body.dfclv * 100 || '0'),
                    gfcnum: Number(body.gfcnum || '0'),
                    dfcnum: Number(body.dfcnum || '0'),
                    gexclv: Number(body.gexclv * 100 || '0'),
                    dexclv: Number(body.dexclv * 100 || '0'),
