...
describe('SkillList', () => {
  ...
  it('renders skill list item with correct data', () => {
    render(<SkillList skill={mockSkill} index={0} />);
  ...
  it('applies correct background class based on index', () => {
    const { rerender } = render(<SkillList skill={mockSkill} index={0} />);
    expect(screen.getByText('Test Music').closest('section')).toHaveClass('bg-blue-200');

    rerender(<SkillList skill={mockSkill} index={1} />);
